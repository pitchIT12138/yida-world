...
Svelte 5 阅读宿主、17 篇精选原文输入、知乎知识／故事导入、富文本粘贴与回答链接／ID 读取尝试、原图保存与离线 ZIP、经济／强模型配置、真实模型请求、段落选区、产物校验、一次修复、iframe 启动检查、手动设计讲解、片段之间与正文的联动、SQLite 后台持久化、可编辑原文、想法自动保存、版本历史和产物导入导出。
...
完整备份、失败恢复、部署边界见 [工作区说明](docs/workspace.md)。当前是本地后台，尚未接入云端数据库；生成进行中刷新不会自动续跑。
