<script lang="ts">
  ...
  import {realAnswersFirst,publicReadingAnswers} from './lib/answer-order';
  ...
  let publishedAnswers=$state<Answer[]>([]);
  ...
  async function loadFeed(){feedBusy=true;try{const r=await fetch('/api/feed?offset='+(feedNext||0));if(!r.ok)throw Error('交互新作暂时无法读取');const data=await r.json();for(const a of data.items){validateSource(a.source);validateArtifact(a.artifact,a.source)}publishedAnswers=[...publishedAnswers.filter((a:Answer)=>!data.items.some((b:Answer)=>b.source.id===a.source.id)),...data.items];publicIds=[...new Set([...publicIds,...data.items.map((a:Answer)=>a.source.id)])];answers=[...data.items.filter((a:Answer)=>!myIds.includes(a.source.id)),...answers.filter(a=>!data.items.some((b:Answer)=>b.source.id===a.source.id)||myIds.includes(a.source.id))];feedNext=data.next}catch(e){notify(String(e))}finally{feedBusy=false}}
  function queueSave(value:Answer,kind:string){
    if(config.authRequired&&!config.user){notify('登录后可以保存自己的作品。');return Promise.resolve()}
    if(!myIds.includes(value.source.id))myIds=[...myIds,value.source.id];
  ...
  let publicReading=$derived(publicReadingAnswers(seedAnswers,publishedAnswers));
  let filtered=$derived((tab==='mine'?answers.filter(a=>myIds.includes(a.source.id)):publicReading).filter(a=>(tab!=='new'||publicIds.includes(a.source.id))&&(tab!=='generated'||!!a.artifact)&&(!query.trim()||[a.source.title,a.source.author,a.tag,...a.source.paragraphs.map(p=>p.text)].join(' ').toLowerCase().includes(query.trim().toLowerCase()))));
  let generated=$derived(publicReading.filter(a=>!!a.artifact).length);
  ...
  function update(id:string,artifact:AnswerArtifact){tab='mine';const old=answers.find(a=>a.source.id===id);ensurePrevious(old);clearTimeout(draftTimers.get(id));answers=answers.map(a=>a.source.id===id?{...a,artifact,reference:undefined}:a);void persist();void queueSave(answers.find(a=>a.source.id===id)!,'generation')}
  function add(input:Answer){
    ...
    answers=[answer,...answers.filter(a=>a.source.id!==answer.source.id)];composer=false;tab='mine';query='';void persist();void queueSave(answer,editId?'source':'import');notify(editId?'原文已更新，之前的版本保留在历史中。':'原文已加入本页并提交后台保存。');
  ...
  async function goToCase(id:string){tab='all';query='';await tick();document.getElementById(id)?.scrollIntoView({block:'start',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'})}
  ...
  function reviewSource(answer:Answer){startWriting();if(composer)reviewing=answer}
  onMount(()=>{
    ...
    void(async()=>{
      ...
      try{
        myIds=[...new Set([...myIds,...local.map(a=>a.source.id)])];
...
<section class="launch-intro"><p>让回答里的知识，变成你能亲手探索的世界。</p><span>知乎黑客松 2026 · 独立参赛作品</span></section>
<nav class="case-nav" aria-label="真实回答直达">{#each ['DLSS 原图','人口与供养','现金流','租售比','国考数据'] as label,i}<a href={'#'+publicReading[i]?.source.id} onclick={e=>{e.preventDefault();void goToCase(publicReading[i].source.id)}}>{label}<span>↗</span></a>{/each}</nav>
<main class="page-grid">
  <div class="feed">
    {#if introOpen}
      <section class="intro-note"><div><span class="eyebrow">一答一世界</span><h2>文字没有说完的，让程序继续表达。</h2><p>Agent 阅读文章，用代码创造适合它的视觉和交互。每个结果都可以回到原文、看到加工说明，再用一句话改变表达。</p><p>精选由当前会话创作并经过操作检查。公共阅读展示发布版本，个人旧副本与草稿保留在“我的作品”。原文和历史可以回看。</p></div><button class="icon-button" aria-label="关闭说明" onclick={()=>introOpen=false}><Icon name="close"/></button></section>
    {/if}
  ...
  <aside class="sidebar launch-sidebar">
