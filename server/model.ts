...
export async function callModel(p:ModelProfile,system:string,input:string,signal:AbortSignal,maxTokens:number):Promise<{text:string;usage:Record<string,number>}>{
  ...
  try{response=await fetch(url,{method:'POST',headers,body:JSON.stringify(body),signal})}catch(e){if(signal.aborted)throw new Error('生成超时或已取消');throw new Error('无法连接模型服务，请检查服务端接口配置')}
  if(!response.ok){
    await response.body?.cancel();
    throw new Error(response.status===429?'模型服务限流或额度不足，请稍后重试':response.status===401||response.status===403?'模型服务鉴权失败，请检查服务端配置':'模型服务请求失败（HTTP '+response.status+'）');
  }
  const length=Number(response.headers.get('content-length'));
  if(length>1000000){await response.body?.cancel();throw new Error('模型结果超出大小限制')}
  const reader=response.body!.getReader();let raw='',bytes=0;const dec=new TextDecoder();
  for(;;){const {done,value}=await reader.read();if(done)break;bytes+=value.length;if(bytes>1000000){await reader.cancel();throw new Error('模型结果超出大小限制')}raw+=dec.decode(value,{stream:true})}
  raw+=dec.decode();
  let data:any;try{data=JSON.parse(raw)}catch{throw new Error('模型服务响应不是有效 JSON')}
  if(data.choices?.[0]?.finish_reason==='length')throw new Error('模型输出达到 token 上限，未生成完整结果；请调整推理模式或输出预算。');
  const text=anthropic?data.content?.filter((x:any)=>x.type==='text').map((x:any)=>x.text).join('\n'):data.choices?.[0]?.message?.content;
  if(typeof text!=='string'||!text.trim())throw new Error('模型没有返回可用内容');
