...
export function applyArtifactPatches(base:unknown,patches:unknown):any{
 ...
 for(const p of patches){
  ...
  else{if(p.op!=='add'&&!Object.hasOwn(parent,key))throw Error('修复补丁目标不存在');if(p.op==='remove')delete parent[key];else parent[key]=structuredClone(p.value)}
 }return value;
}
export const PATCH_INSTRUCTION='当前 candidate 是上一次产物。优先只返回JSON {"patches":[{"op":"replace","path":"/blocks/0/js","value":"修好的完整该字段字符串"}]}，只修改错误所需字段，避免重新输出正确代码。支持add/replace/remove及数组下标，允许根字段blocks/bindings/scene/design/explanation/libraryReferences/version/answerId；不得改provenance。路径必须是candidate实际结构。若不存在可解析candidate才返回完整artifact。补丁由服务端合入后仍执行全部校验。';
