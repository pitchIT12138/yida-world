...
describe('real generation transport with explicitly mocked provider',()=>{
  ...
  it('records server provenance and actual provider usage',async()=>{
    const fetcher=vi.fn().mockResolvedValue(mock(JSON.stringify(fixture())));vi.stubGlobal('fetch',fetcher);
    const r=await post(input),e=events(await r.text()),result=e.find(x=>x.type==='result');
    expect(fetcher).toHaveBeenCalledTimes(1);expect(result.artifact.provenance.model).toBe('mock-model');
    ...
    expect(result.artifact.provenance.usage.completion_tokens).toBe(20);
  });
  ...
  it('structural repair consumes the one of the two repairs',async()=>{
    const fetcher=vi.fn().mockResolvedValueOnce(mock('bad')).mockResolvedValueOnce(mock(JSON.stringify(fixture())));vi.stubGlobal('fetch',fetcher);
    const e=events(await(await post(input)).text());expect(fetcher).toHaveBeenCalledTimes(2);
  ...
  it('accepts one runtime repair with the signed original candidate',async()=>{
    const fetcher=vi.fn().mockImplementation(()=>Promise.resolve(mock(JSON.stringify(fixture()))));vi.stubGlobal('fetch',fetcher);
    ...
    expect(repaired.type).toBe('result');expect(repaired.repairTicket).toBeTruthy();expect(fetcher).toHaveBeenCalledTimes(2);
  });
  it('rejects a changed candidate before making a repair call',async()=>{
    ...
    expect(r.status).toBe(400);expect(fetcher).toHaveBeenCalledTimes(1);
  });
