...
try{
 ...
 assert.equal(mediaRequests.length,0,'Offscreen original image bundles must not load on first paint');
 ...
 await card.getByRole('button',{name:'只看原文',exact:true}).click();
 ...
 await card.getByRole('button',{name:'显示交互',exact:true}).click();
 await block.scrollIntoViewIfNeeded();
 await iframe.locator('#both').click();
