...
export async function requestGeneration(base:string,input:GenerationInput):Promise<AnswerArtifact>{
  const response=await fetch(base.replace(/\/$/,'')+'/api/generate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(input),signal:AbortSignal.timeout(610000)});
