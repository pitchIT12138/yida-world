...
const env={...process.env,...await readFile('.dev.vars','utf8').then(parse).catch(()=>({}))};
...
const dir='artifacts/pipeline/'+new Date().toISOString().replace(/[:.]/g,'-');await mkdir(dir,{recursive:true});const events:unknown[]=[];
try{
 ...
 for(const item of selected){const id=String(item.work_id);if(!/^\d{1,30}$/.test(id))continue;const url='https://api.zhihu.com/km-indep-home/hackathon/v2/story/'+id;const response=await fetch(url,{signal:AbortSignal.timeout(20000)});if(!response.ok)throw Error('官方正文读取失败');const raw=await response.json();if(typeof raw.content!=='string'||raw.content.length<100)throw Error('接口未提供可用正文');
  ...
  events.push(await api('/api/admin/jobs',input));
 }
 ...
 if(events.some((e:any)=>e.deduplicated&&e.status==='failed'))process.exitCode=1;
}finally{await writeFile(dir+'/run.json',JSON.stringify({createdAt:new Date().toISOString(),events},null,2));console.log(JSON.stringify(events))}
