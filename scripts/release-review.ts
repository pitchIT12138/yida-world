...
export async function reviewCandidate(source:AnswerSource,artifact:AnswerArtifact,assets:SourceAsset[],env:RuntimeEnv,outDir:string){
 ...
 await mkdir(outDir,{recursive:true});await writeFile(outDir+'/review-model.json',JSON.stringify({text:response.text,usage:response.usage,model:p.model},null,2));
 let transport;try{transport=modelJSON(response.text)}catch(e){throw new ReviewUnavailable('验收模型未返回可解析JSON，保留候选：'+String(e))}
